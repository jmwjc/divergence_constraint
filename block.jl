...
using ApproxOperator.Elasticity: ∫εᵢⱼσᵢⱼdΩ, ∫vᵢbᵢdΩ, ∫vᵢtᵢdΓ, ∫vᵢgᵢdΓ, L₂
...
∂σ₁₁∂x(x,y,z) = E/(1+ν)/(1-2*ν)*((1-ν)*∂ε₁₁∂x(x,y,z) + ν*∂ε₂₂∂x(x,y,z))
∂σ₁₁∂y(x,y,z) = E/(1+ν)/(1-2*ν)*((1-ν)*∂ε₁₁∂y(x,y,z) + ν*∂ε₂₂∂y(x,y,z))
∂σ₁₁∂z(x,y,z) = E/(1+ν)/(1-2*ν)*((1-ν)*∂ε₁₁∂y(x,y,z) + ν*∂ε₂₂∂y(x,y,z))
∂σ₂₂∂x(x,y,z) = E/(1+ν)/(1-2*ν)*(ν*∂ε₁₁∂x(x,y,z) + (1-ν)*∂ε₂₂∂x(x,y,z))
∂σ₂₂∂y(x,y,z) = E/(1+ν)/(1-2*ν)*(ν*∂ε₁₁∂y(x,y,z) + (1-ν)*∂ε₂₂∂y(x,y,z))
∂σ₂₂∂z(x,y,z) = E/(1+ν)/(1-2*ν)*(ν*∂ε₁₁∂y(x,y,z) + (1-ν)*∂ε₂₂∂y(x,y,z))
∂σ₃₃∂x(x,y,z) = E/(1+ν)/(1-2*ν)*(ν*∂ε₁₁∂x(x,y,z) + (1-ν)*∂ε₂₂∂x(x,y,z))
∂σ₃₃∂y(x,y,z) = E/(1+ν)/(1-2*ν)*(ν*∂ε₁₁∂y(x,y,z) + (1-ν)*∂ε₂₂∂y(x,y,z))
∂σ₃₃∂z(x,y,z) = E/(1+ν)/(1-2*ν)*(ν*∂ε₁₁∂y(x,y,z) + (1-ν)*∂ε₂₂∂y(x,y,z))
...
prescribe!(elements["Γʳ"],:t₂=>(x,y,z,n₁,n₂,n₃)->σ₁₂(x,y,z)*n₁+σ₂₂(x,y,z)*n₂+σ₂₃(x,y,z)*n₃)
prescribe!(elements["Γʳ"],:t₃=>(x,y,z,n₁,n₂,n₃)->σ₁₃(x,y,z)*n₁+σ₂₃(x,y,z)*n₂+σ₃₃(x,y,z)*n₃)
prescribe!(elements["Γʳ"],:α=>(x,y,z)->1e9*E)
...
prescribe!(elements["Ωᵍ"],:v=>v)
prescribe!(elements["Ωᵍ"],:w=>v)
prescribe!(elements["Ωᵍ"],:∂u∂x=>∂u∂x)
prescribe!(elements["Ωᵍ"],:∂u∂y=>∂u∂y)
prescribe!(elements["Ωᵍ"],:∂u∂z=>∂u∂z)
prescribe!(elements["Ωᵍ"],:∂v∂x=>∂v∂x)
prescribe!(elements["Ωᵍ"],:∂v∂y=>∂v∂y)
prescribe!(elements["Ωᵍ"],:∂v∂z=>∂v∂z)
prescribe!(elements["Ωᵍ"],:∂w∂x=>∂w∂x)
prescribe!(elements["Ωᵍ"],:∂w∂y=>∂w∂y)
prescribe!(elements["Ωᵍ"],:∂w∂z=>∂w∂z)
